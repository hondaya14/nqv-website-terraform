...
resource "aws_cloudfront_distribution" "resume_s3_distribution" {
  origin {
    ...
    origin_access_control_id = aws_cloudfront_origin_access_control.resume_origin_access_control.id
  ...
  viewer_certificate {
    acm_certificate_arn      = var.sub_acm_certificate_arn
    minimum_protocol_version = "TLSv1.2_2021"
    ssl_support_method       = "sni-only"
  }
}

resource "aws_cloudfront_origin_access_control" "resume_origin_access_control" {
  name                              = "resume_origin_access_control"
  origin_access_control_origin_type = "s3"
  signing_behavior                  = "always"
  signing_protocol                  = "sigv4"
}
